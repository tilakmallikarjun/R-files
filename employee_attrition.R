...
#train_data<-subset(train_data, DaysInCompany < 200)
#encoding data
...
#testfull_df<-train_data %>% filter(MMM.YY >= "2017-07-01")
#test_df<-testfull_df %>% 
 # group_by(Emp_ID) %>%
  #dplyr::slice(which.max(as.Date(MMM.YY, '%m/%d/%Y')))
...
ggplot(df,aes(Quarterly.Rating,fill=attrition))+geom_bar()
...
rpart.tree <- rpart(attrition ~ ., data=df[,!names(df) %in% "Emp_ID"])
...
predictions <- predict(rpart.tree, test_df, type="class")
conf_df <- data.frame(table(test_df$attrition, predictions))
ggplot(data =  conf_df, mapping = aes(x = predictions, y = Var1)) +
  geom_tile(aes(fill = Freq), colour = "white") +
  geom_text(aes(label = sprintf("%1.0f", Freq)), vjust = 1) +
...
##########RANDOM FOREST####################
set.seed(1) 
bag.attrition=randomForest(attrition ~ ., data=df[,!names(df) %in% "Emp_ID"], mtry=3,importance=TRUE) 
summary(bag.attrition)


##TESTING####
predictions = predict(bag.attrition, newdata=test_df,type = "class")

conf_df <- data.frame(table(test_df$attrition, predictions))
confusion_matrix_rf <- table(test_df$attrition, predictions)
confusion_matrix_rf
